...
orientationPermissionPage opts = D.div (oneOf [ pure (D.Class := "absolute") ])
  [ D.div (oneOf [ pure (D.Class := "z-10 absolute grid grid-cols-3 grid-rows-3  place-items-center h-screen w-screen") ])
      [ D.div (klass_ "col-start-1 col-end-4 row-start-1 row-end-2")
          [ D.p (klass_ "text-center text-2xl text-slate-800 p-3") [ text_ "To play Joyride, you need to ask for orientation permissions." ]

          ]
      , D.button
          ( oneOf
              [ klass_ "text-center bg-gray-600 hover:bg-gray-400 text-white py-2 px-4 rounded col-start-2 col-end-2 row-start-2 row-end-3"
