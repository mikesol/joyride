...
sandbox :: Nut
sandbox = D.div (oneOf [ bang $ D.Class := "h-screen w-screen bg-zinc-900 absolute grid grid-cols-6 grid-rows-6" ])
  [ D.div (oneOf [ bang $ D.Class := "col-start-2 col-end-6 row-start-2 row-end-6 flex flex-col justify-items-center overflow-scroll text-center" ])
                  ...
                  D.li_
                    [ D.button
                        ( oneOf
                            ...
                            , DL.click (bang (pure unit :: Effect Unit))
                            ]
                        )
